...
def get_sa_for_correct_and_incorrect(sa_score, correct_index, incorrect_index, dataset, adversarial):
...
def draw_box_plot(sa_correct, sa_incorrect, dataset, type_sa, adversarial):    
    ...
    if adversarial:
        plt.savefig('./figures/{}_adversarial_{}.jpg'.format(type_sa, dataset))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--adv_prob", "-adv_prob", help="Calculate confidence score of adversarial examples", action="store_true"
    )
    ...
    if args.d == "mnist":
        if args.adv:
            ...
            sa_correct, sa_incorrect = get_sa_for_correct_and_incorrect(sa_score=sa_score, correct_index=correct, incorrect_index=incorrect, dataset=args.d, adversarial=args.adv)
            ...
            draw_box_plot(sa_correct=sa_correct, sa_incorrect=sa_incorrect, dataset=args.d, type_sa=type_sa, adversarial=args.adv)
            ...
            prob_pred_adv = list(np.amax(y_pred_adv, axis=1))

            type_sa = 'prob'

            sa_correct, sa_incorrect = get_sa_for_correct_and_incorrect(sa_score=prob_pred_adv, correct_index=correct, incorrect_index=incorrect, dataset=args.d, adversarial=True)
            print('Dataset: %s' % (args.d))
            print('Mean and std of correct instances for %s score: %.3f, %.3f' % (type_sa, np.mean(sa_correct), np.std(sa_correct)))
            print('Mean and std of incorrect instances for %s score: %.3f, %.3f' % (type_sa, np.mean(sa_incorrect), np.std(sa_incorrect)))
            print('Accuracy of cifar dataset: %.3f' % (len(sa_correct) / (len(sa_correct) + len(sa_incorrect))))
            draw_box_plot(sa_correct=sa_correct, sa_incorrect=sa_incorrect, dataset=args.d, type_sa=type_sa, adversarial=True)
