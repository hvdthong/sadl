...
def mnist_get_correct_and_incorrect_test_images(y_pred, y_true):
    ...
    for row in range(nrows):
        if np.array_equal(y_pred[row, :], y_true[row, :]):
            correct.append(row)
        else:
...
if __name__ == '__main__':
    ...
    if args.d == "mnist":
        ...
        sa_correct, sa_incorrect = get_sa_for_correct_and_incorrect(sa_score=sa_score, correct_index=correct, incorrect_index=incorrect)
